...
- Connexions actives: `OnlinePresenceService` (Scoped, `IAsyncDisposable`) — heartbeat Redis 10 s / TTL 20 s; clau `autoco:online:prof:{id}` o `autoco:online:stu:{id}` (JSON `OnlineUserSnapshot`); iniciat a `MainLayout.OnAfterRenderAsync` quan `IsLoggedIn`; aturat a `Logout` i `DisposeAsync`; `LoginResponse` porta `ClassId?`/`ClassName?` per a sessions d'alumne; `IServer.Keys("autoco:online:*")` per escanejar connexions actives; pàgina `/admin/connexions` accessible a Admin i Gestor
