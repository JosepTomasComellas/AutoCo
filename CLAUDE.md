...
- Accés a activitats per professors assignats via `ProfessorClass`: `ActivityService.CreateAsync` comprova `m.ProfessorId == professorId OR db.ProfessorClasses.Any(pc => pc.ProfessorId == professorId && pc.ClassId == m.ClassId)` — un professor assignat a una classe pot crear activitats en qualsevol mòdul d'aquella classe encara que no n'hagi estat el creador
